...
import (
	...
	"supplier-management/util"

	"github.com/gin-gonic/gin"
)

type BaseHandlerInterface[T any] interface {
...
func NewBaseHandler[T any](service baseService.BaseServiceInterface[T]) *BaseHandler[T] {
	return &BaseHandler[T]{Service: service}
}

func (ctrl *BaseHandler[T]) Create(c *gin.Context) {
	var entity T
...
func (ctrl *BaseHandler[T]) GetAll(c *gin.Context) {
	var entities []T
...
func (ctrl *BaseHandler[T]) GetByUUID(c *gin.Context) {
	uuid := c.Param("uuid")
	if uuid == "" {
...
func (ctrl *BaseHandler[T]) Update(c *gin.Context) {
	uuid := c.Param("uuid")
	if uuid == "" {
		util.ErrorBadRequest(c, "UUID parameter is required")
		return
...
func (ctrl *BaseHandler[T]) Delete(c *gin.Context) {
	uuid := c.Param("uuid")
	if uuid == "" {
		util.ErrorBadRequest(c, "UUID parameter is required")
		return
...
func (ctrl *BaseHandler[T]) SoftDelete(c *gin.Context) {
	uuid := c.Param("uuid")
	if uuid == "" {
		util.ErrorBadRequest(c, "UUID parameter is required")
		return
...
func (ctrl *BaseHandler[T]) GetPagination(c *gin.Context) {
...
func (ctrl *BaseHandler[T]) GetByField(c *gin.Context) {
	field := c.Query("field")
	value := c.Query("value")
...
func (ctrl *BaseHandler[T]) FindByName(c *gin.Context) {
	name := c.Query("name")
	if name == "" {
